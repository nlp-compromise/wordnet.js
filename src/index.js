...
//handle all pos-methods
const doit = (data, str) => {
  //lookup
  if (typeof str === 'string') {
    return data.filter((o) => {
      return o.words.indexOf(str) !== -1
    })
  }
  if (str instanceof RegExp) {
    return data.filter((o) => {
      return o.words.find((w) => w.match(str))
    })
  }
  //list
  let obj = {}
  for (let i = 0; i < data.length; i++) {
    for (let o = 0; o < data[i].words.length; o++) {
      obj[data[i].words[o]] = true
    }
  }
...
exports.adverbs = function(str) {
  let data = require('../data/Adverb').data;
  return doit(data, str)
}
exports.verbs = function(str) {
  let data = require('../data/Verb').data;
  return doit(data, str)
}
exports.nouns = function(str) {
  let data = require('../data/Noun').data;
  return doit(data, str)
}
exports.adjectives = function(str) {
  let data = require('../data/Adjective').data;
  return doit(data, str)
}
//
